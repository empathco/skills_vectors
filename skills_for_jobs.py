...
def save_job_skills_weaviate(job_skills,best_skills,best_vector,filename):
...
def save_job_skills_milvus(job_skills,best_skills,best_vector,filename):
    rows=[]
    tot_quality = 0
    similarities=[]
    avg_similarities=[]
...
def save_job_skills_pg(job_skills,best_skills,best_vector,filename):
    rows=[]
    tot_quality = 0 
    similarities = []
    avg_similarities=[]
...
def get_nearest_neighbor_skills(cursor,job_vec):
    ...
    query += "SELECT abbreviation,embedding <=> '[" + vec_str +"]' AS score,embedding FROM skills ORDER BY score DESC LIMIT "+str(MAX_SKILLS*10) 
    #query += ";COMMIT;"
    #print(f"Query {query}")
    start = time.time()
    cursor.execute(query)
...
save_job_skills_weaviate(job_skills_weaviate,best_skills,best_vector,'job_skills_weaviate.csv')
...
save_job_skills_milvus(job_skills_milvus,best_skills,best_vector,'job_skills_milvus.csv')
...
save_job_skills_pg(job_skills_pg,best_skills,best_vector,'job_skills_pg.csv')